...
import { FETCH_TV_SHOW_DETAILS, FETCH_TV_SHOW_CREDITS, FETCH_TV_SHOW_REVIEWS, FETCH_SIMILAR_TV_SHOWS } from '../constants';
import { fetchTVShowDetailsApi, fetchTVShowCreditsApi, ffetchTVShowReviewsApi, fetchSimilarTVShowsApi } from '../../../api';
import { API_KEY, LANGUAGE } from '../../../constants';
import { fetchTVShowDetailsSuccessful, fetchTVShowDetailsFailed, fetchTVShowCreditsSuccessful, fetchTVShowCreditsFailed, fetchTVShowReviewsSuccessful, fetchTVShowReviewsFailed, fetchSimilarTVShowsSuccessful, fetchSimilarTVShowsFailed } from '../actions';

const params = {
	params: { api_key: API_KEY, language: LANGUAGE }
};

const id = ({ tvShowDetails }) => tvShowDetails.tvShowId

function* fetchTVShowDetailsAsync() {
    try {
        const tvShowId = yield select(id);
...
function* fetchTVShowCreditsAsync() {
    try {
        const tvShowId = yield select(id);
        const { data: { cast } } = yield call(fetchTVShowCreditsApi, tvShowId, params);
...
function* fetchTVShowReviewsAsync() {
    try {
        const tvShowId = yield select(id);
        const { data: { results } } = yield call(ffetchTVShowReviewsApi, tvShowId, params);
...
function* fetchSimilarTVShowsAsync() {
    try {
        const tvShowId = yield select(id);
        const { data: { results } } = yield call(fetchSimilarTVShowsApi, tvShowId, params);
